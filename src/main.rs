use std::{fs::File, io::Read, hash::Hash, ops::Range};
...
pub enum Declaration<'a> {
    ...
    Function { name: String, args: Vec<NameTypePair<'a>>, return_type: Option<Type<'a>>, block: Vec<Statement<'a>>, loc: Loc<'a> },
...
impl<'a> Declaration<'a> {
    fn loc(&self) -> Loc<'a> {
        *match self {
            ...
            Declaration::Function { name, args, return_type, block, loc } => loc,
...
peg::parser!{
    grammar lexer(file: &'input UwUInpFile) for str {
        rule ___ = [' ' | '\n']*
        rule __ = "//" [^'\n']*
        rule _ = ___ __? ___
...
peg::parser!{
    grammar uwu_parser<'a>() for [Token<'a>] {
        ...
        = [Token::Keyword(Keyword::Import, left)] paths:([Token::Identifier(_, _)] ** [Token::Symbol(Symbol::Dot, _)]) [Token::Symbol(Symbol::Colon, _)][Token::Symbol(Symbol::Colon, _)] [Token::Identifier(name, _)] [Token::Symbol(Symbol::Semicolon, right)] { Declaration::Import { path: paths.iter().map(|x| match x { Token::Identifier(v, _) => v.to_string(), _ => panic!() }).collect(), item: name.to_string(), loc: left.to(&right) } }

        rule function_ret() -> Declaration<'a>
        = [Token::Keyword(Keyword::Fun, left)] [Token::Identifier(name, _)] [Token::Symbol(Symbol::OpenParen, _)] args:(name_type_pair() ** [Token::Symbol(Symbol::Comma, _)]) [Token::Symbol(Symbol::CloseParen, _)] [Token::Symbol(Symbol::Colon, _)] return_type: tpe() block:braceblock() right:position!() { Declaration::Function { name: name.to_string(), args, return_type: Some(return_type), block: block.0, loc: left.to(&Loc { file: left.file, left: left.left, right }) } }

        rule function_no_ret() -> Declaration<'a>
        = [Token::Keyword(Keyword::Fun, left)] [Token::Identifier(name, _)] [Token::Symbol(Symbol::OpenParen, _)] args:(name_type_pair() ** [Token::Symbol(Symbol::Comma, _)]) [Token::Symbol(Symbol::CloseParen, _)] block:braceblock() right:position!() { Declaration::Function { name: name.to_string(), args, return_type: None, block: block.0, loc: left.to(&Loc { file: left.file, left: left.left, right }) } }

        rule function() -> Declaration<'a>
        = function_ret() / function_no_ret()

        rule strct() -> Declaration<'a>
        = [Token::Keyword(Keyword::Struct, left)] [Token::Identifier(name, _)] [Token::Symbol(Symbol::OpenBrace, _)] fields:(name_type_pair() ** [Token::Symbol(Symbol::Comma, _)]) [Token::Symbol(Symbol::CloseBrace, right)] { Declaration::Struct { name: name.to_string(), fields, loc: left.to(&right) } }
